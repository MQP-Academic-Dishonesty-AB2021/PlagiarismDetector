...
(define BST1 (make-bst W1 (make-bst A1 false
...
(check-expect (smaller? "A1" false) "error: false bst") 
(check-expect (smaller? "A1" (make-bst W1 false false)) true)  
(check-expect (smaller? "D1" (make-bst A1 false false)) false) 
(check-expect (smaller? "D1"
...
(check-expect (same? "W1"
                     (make-bst A1 (make-bst W1 false false) false))
              false) 
(check-expect (same? "Z1" false) "error: false bst") 
...
(check-expect (find-name "X1" BST1) false) 
      

(define (find-name key bst)
  (cond [(false? bst) false]
        [else
         (cond [(same? key bst) (bst-widget bst)]
               [(smaller? key bst)
...
(check-expect (insert-name A1
                           (make-bst W1 (make-bst D1 false false) false))
              (make-bst W1 (make-bst D1 (make-bst A1 false false) false) false))

(check-expect (insert-name Z1
                           (make-bst D1
        ...
        [else
         (if (smaller? (widget-name v) b)
             (make-bst (bst-widget b)  
                       (insert-name v (bst-left b)) (bst-right b))
             (make-bst (bst-widget b)
                       (bst-left b) (insert-name v (bst-right b))))]))
