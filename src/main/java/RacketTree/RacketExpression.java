...
public abstract class RacketExpression {
	 ...
	 * @throws InvalidFormatException The file does not follow proper racket syntax
	 *                                and a next racket expression could not be
	 *                                parsed
	 * @throws IOException            There was an error reading the file
	 */
	public static RacketExpression generateAtom(PushbackReader fin, RacketList parent)
			throws InvalidFormatException, IOException {
		int firstChar = fin.read();
		while (Character.isWhitespace((char) firstChar)) {
			firstChar = fin.read();
		}
		switch (firstChar) {
			case -1:
	 ...
	 * @return A value representing the similarity; Based off the height of the tree
	 *         containing both this expression and the other
	 */
	 ...
	 * 
	 * @param map The map containing the expressions to generate similarity values
	 *            from
