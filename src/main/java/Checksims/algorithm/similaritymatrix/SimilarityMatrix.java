...
import Checksims.submission.NoSuchSubmissionException;
...
/**
 * A Similarity Matrix represents the similarities between a given group of
 * submissions.
 *
 * TODO consider offering Iterators for the entire similarity matrix, and for
 * individual submissions on the X axis
 */
public final class SimilarityMatrix {
	...
	/**
	 * Create a Similarity Matrix with given parameters. Internal constructor used
	 * by factory methods.
	 *
	 * Lists, not sets, of submissions, to ensure we have an ordering. We maintain
	 * the invariant that there are no
	...
	/**
	 * @return Get the Algorithm Results that were used to build this similarity
	 *         matrix
	 */
	 ...
	 * @throws NoSuchSubmissionException Thrown if either xSubmission or ySubmission
	 ...
	 * @param results          Results to build from. Must contain results for every
	 *                         possible unordered pair of input submissions
	 * @return Similarity Matrix built from given results
	 * @throws InternalAlgorithmError Thrown on missing results, or results
	 *                                containing a submission not in the input
	 */
	public static SimilarityMatrix generateMatrix(Set<Submission> inputSubmissions, Set<AlgorithmResults> results)
		...
		// Now go through all the results, and build appropriate two MatrixEntry objects
		// for each
	 ...
	 *
	 * The result is not a square matrix. Only the input submissions are on the X
	 * axis, but the Y axis contains both
	 * input and archive submissions.
	 *
	 * @param inputSubmissions   Submissions used to generate matrix
	 * @param archiveSubmissions Archive submissions - only compared to input
	 *                           submissions, not to each other
