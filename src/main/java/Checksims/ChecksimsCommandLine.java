...
import Checksims.algorithm.preprocessor.CommonCodeLineRemovalPreprocessor;
import Checksims.algorithm.preprocessor.PreprocessorRegistry;
 ...
 *
 * TODO: Consider changing from a static class? Having the CommandLine as an
 * instance variable would greatly simplify
 */
public final class ChecksimsCommandLine {
	...
	/**
	 * @param level Logging level to use. Supported levels are 0 (nonverbose), 1
	 ...
	 * @return Modified baseConfig with submissions (and possibly common code and
	 *         archive submissions) changed
	 ...
	 * @param recursive      Whether to recursively traverse when building
	 *                       submissions
	 * @return Collection of submissions which will be used to run Checksims
	 * @throws IOException Thrown on issue reading files or traversing directories
	 ...
	 * @throws ChecksimsException Thrown on invalid CLI arguments or error running
	 *                            Checksims
	 * @throws IOException        Thrown on error building a submission from files
	 *                            or writing output to file
	 */
	public static void runCLI(String[] args) throws ParseException, ChecksimsException, IOException {
		checkNotNull(args);

		// Parse options, first round: nothing required, so we can check for --help and
		// --version
		...
		if (cli.hasOption("f")) {
			...
			for (String strategy : output.keySet()) {
				// Final filename is the basename specified through CLI, with the strategy name
				// as its extension.
