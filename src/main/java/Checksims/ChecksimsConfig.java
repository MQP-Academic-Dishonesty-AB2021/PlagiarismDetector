...
import Checksims.algorithm.similaritymatrix.output.MatrixPrinter;
import Checksims.algorithm.similaritymatrix.output.MatrixPrinterRegistry;
 ...
 *
 * This configuration contains all of the information needed to run Checksims on
 * a number of submissions and return
 * meaningful output.
 *
 * All Setter methods return the current configuration, to allow chaining.
 *
 * TODO: add a setImmutable method (or an immutable wrapper?) so ChecksimsRunner
 * cannot alter a running config
 */
public final class ChecksimsConfig {
	 ...
	 *
	 * The default configuration is complete, and all fields are initialized to
	 * reasonable values. The only thing
	 * required to be set before running Checksims with a default config are the
	 * submissions to be run on, which
	 * default to empty --- ChecksimsRunner will throw an exception if run with no
	 * submissions.
	 */
	...
	public ChecksimsConfig setPreprocessors(List<SubmissionPreprocessor> newPreprocessors) {
		...
		// Can't use a set, we don't require preprocessors to implement equals() or
		// hashCode() in sane ways
	...
	/**
	 * @param newSubmissions New set of submissions to work on. Must contain at
	 *                       least 1 submission.
	 * @return This configuration
	 */
	...
	/**
	 * @param newArchiveSubmissions New set of archive submissions to use. May be
	 *                              empty.
	 * @return This configuration
	 */
	...
	/**
	 * @param newNumThreads Number of threads to be used for parallel operations.
