 ...
 *
 * Tokens are backed by an Object. In the case of Line and Whitespace tokens,
 * this should be a string. In the case of
 * Character tokens, this should be a Character. This is not enforced for
 * performance reasons, but the invariant is
...
public final class ConcreteToken implements Token {
	 ...
	 *
	 * Does not actually use the LexemeMap, and instead uses a directly-provided
	 * lexeme. If the given lexeme is invalid,
	 * it WILL result in a RuntimeException. Hence, this is only used as a copy
	 * constructor, for high-speed duplication
	 * of tokens.
	 *
	 ...
	 * @return True if object compared to is a Token with same type and equiv.
	 *         tokenization value
	 */
	@Override
