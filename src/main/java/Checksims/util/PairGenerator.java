...
public final class PairGenerator {
	...
	public static Set<Pair<Submission, Submission>> generatePairs(Set<Submission> submissions) {
		...
		while (remaining.size() >= 2) {
			...
			// Form a pair for every remaining submission by pairing with the first, removed
			// submission
	...
	/**
	 * Generate all pairs for normal submissions, and pairs for archive submissions
	 * to compare to normal submissions.
	 *
	 * @param submissions        Normal submissions - compared to each other and
	 *                           archive submissions
	 * @param archiveSubmissions Archive submissions - only compared to normal
	 *                           submissions, not each other
	 * @return Set of all unordered pairs required for comparison with archive
	 *         directory
	 */
	public static Set<Pair<Submission, Submission>> generatePairsWithArchive(Set<Submission> submissions,
		...
		// TODO it may be desirable to allow comparison of a single submission to an
		// archive
		...
		// If we have no archive submissions, just return the same result generatePairs
		// would
		...
		while (!remaining.isEmpty()) {
			...
			for (Submission s : submissions) {
				...
				// One pair for each normal submission, consisting of the archive submission and
