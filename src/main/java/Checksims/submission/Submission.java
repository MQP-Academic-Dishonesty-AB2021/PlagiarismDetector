 ...
 *
 * Submissions are considered Comparable so they can be ordered for output.
 * Generally, we only expect that their names,
...
public interface Submission extends Comparable<Submission> {
	 ...
	 *
	 * The directory is assumed to hold a number of subdirectories, each containing
	 * one student or group's submission
	 ...
	 * @param glob      Match pattern used to identify files to include in
	 *                  submission
	 * @param splitter  Tokenizes files to produce Token Lists for a submission
	 * @return Set of submissions including all unique nonempty submissions in the
	 *         given directory
	 * @throws java.io.IOException Thrown on error interacting with file or
	 *                             filesystem
	 */
	 ...
	 *
	 * The contents of a submission are built deterministically by reading in files
	 * in alphabetical order and appending
	 * their contents.
	 *
	 ...
	 * @return A new submission formed from the contents of all given files,
	 *         appended and tokenized
	...
	static Submission submissionFromFiles(String name, Set<File> files, Tokenizer splitter)
		...
		// To ensure submission generation is deterministic, sort files by name, and
		...
		// Could do this with a .stream().forEach(...) but we'd have to handle the
		// IOException inside
